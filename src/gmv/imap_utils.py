...
class GIMAPFetcher(object): #pylint:disable-msg=R0902
    ...
    ALL_FOLDER = (GMAIL_ALL, GOOGLE_MAIL_ALL, u'[Gmail]/\xd6sszes lev\xe9l')
    ...
    def find_all_mail_folder(self):
        ...
        assert folders and folders[-1]
        the_dir = None
        for (_, _, the_dir) in folders:
            if the_dir in GIMAPFetcher.ALL_FOLDER:
                self._all_mail_folder = the_dir
                break

        if not the_dir:
            #Error
            raise Exception("Cannot find global dir in %s. Are you sure it is a GMail account" % \
                            (GIMAPFetcher.ALL_FOLDER))
